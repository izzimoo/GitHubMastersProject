...
#CarBus:
chosenWords = ['engine', 'steer', 'drive', 'seat', 'wheels', 'commute', 'passenger', 'driver', 'motorway', 'taxi','intercity', 'open-top', 'window', 'roof', 'garage']
...
# CatDog: 
#chosenWords = ['fur', 'paw', 'cute', 'cosy', 'playful', 'microchipped', 'neutered', 'pet', 'breed', 'stray', 'whiskers', 'train', 'wild', 'cuddle', 'cosy', 'feed', 'tail', 'lead']
#chosenWords = ['']
...
def forceSequences(meaning1Words, meaning2Words):
...
def reRunPercentile(ambiguousWords, ambigPercentage):
    ...
    for word, percentage in zip(ambiguousWords, ambigPercentage):
        ...
        if  ((((percentage > small) and (percentage < big)) or ((1-percentage > small) and (1-percentage < big))) and (word not in tempAmbiguousWords)): # While they are between the percentiles they are ambiguous words
        ...
        for low in lowPercentile:
            for high in highPercentile:
                ...
                if (noOfIterations > 100):
                    ambiguousWords, ambigPercentage, meaning1Words, meaning1Percentage, meaning2Words, meaning2Percentage = forceSequences(meaning1Words, meaning2Words)
