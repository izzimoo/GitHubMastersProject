...
def test(meaning1Words, meaning2Words):

    #print(percentageToMeaning1); print(chosenWords)

    sortedPercentage, sortedWords = (list(t) for t in zip(*sorted(zip(percentageToMeaning1, chosenWords)))) 
    #print(sortedPercentage); print(sortedWords)

    if ( (len(meaning1Words) < 5) and (len(meaning2Words) < 4)):
        ...
        for low in lowPercentile:
            for high in highPercentile:
                ...
                if (noOfIterations > 100):
                    ambiguousWords, ambigPercentage, meaning1Words, meaning1Percentage, meaning2Words, meaning2Percentage = test(meaning1Words, meaning2Words)
                    exitWhileLoop = 1
                    break
