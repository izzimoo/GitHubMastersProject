...
#CarBus:chosenWords = ['engine', 'drive', 'seat', 'wheels', 'commute', 'passenger', 'driver', 'fuel', 'motorway', 'taxi', 'crash','intercity', 'open-top']
...
# ShirtTShirt: chosenWords = ['button', 'sleeve', 'collar', 'armholes', 'pajamas', 'outfit', 'pullover', 'cotton', 'linen', 'uniform', 'undergarment', 'causal']
# FlowerGrass: chosenWords = ['garden', 'grow', 'weed', 'ground', 'roots', 'die', 'meadow', 'stem', 'aphid', 'green', 'seeds', 'pollen', 'leaves', 'pasture', 'dirt', 'water', 'foliage', 'spring']
...
while ( (len(meaning1Words) < 4) or (len(meaning2Words) < 4) or (len(ambiguousWords) < 4) ): # while all the arrays have less than 4 words in them

    if (exitWhileLoop == 1):
        break
    
    highPercentile = range(60, 80, 5) #50,70 50,70
    lowPercentile = range(30, 40, 5) #30,40 10,30
    
    if (increasePercentileStep == 1):
        highPercentile = range(50, 70, 10)
        lowPercentile = range(30, 40, 10)
    ...
    for low in lowPercentile:
        for high in highPercentile:
            ...
            noOfIterations+=1
            if ( (noOfIterations >= 40) and (forLoopBroke == 0) ): # Likely entering an infinite loop so break out of for loop and increase the percentile step from 5 to 10
                increasePercentileStep = 1; forLoopBroke = 1
                break


            if (len(meaning1Words) >= 4) and (len(meaning2Words) >= 4) and (len(ambiguousWords) >= 4):
                for testing in ambigPercentage:
                    if (testing >= 0.56):
                        lowPercentile = range(50, 70, 5) #50,70 50,70
                
                for testing2 in meaning2Percentage:
                    if (testing2 <=0.53):
                        lowPercentile = range(10, 30, 5)
                #exitWhileLoop = 1
                break

            #print(debug1); print(debug2); print(debug3)
